          ...
          dangerouslySetInnerHTML={{
            __html: `
              ...
              (function prepareDiagnosisSession(){
                ...
                function formatMoney(value){ if (!Number.isFinite(value)) return 'no calculable'; return new Intl.NumberFormat('es-ES', { style:'currency', currency:'EUR', maximumFractionDigits:0 }).format(value); }
                function readNumber(text){
                  if (!text) return null; var raw = String(text).trim().replace(/[−–—]/g, '-'); var cleaned = raw.replace(/[^0-9,.-]/g, ''); if (!cleaned) return null;
                  var hasComma = cleaned.indexOf(',') > -1; var hasDot = cleaned.indexOf('.') > -1;
                  if (hasComma && hasDot) cleaned = cleaned.replace(/\./g, '').replace(',', '.'); else if (hasComma) cleaned = cleaned.replace(',', '.');
                  var n = Number(cleaned); return Number.isFinite(n) ? n : null;
                }
                ...
                function applyPositiveEconomicExamples(){
                  var examples = [['Ingresos o margen potencial estimado','500.000'],['Porcentaje estimado que podria capturarse','70'],['Eficiencia economica potencial','150.000'],['Porcentaje estimado de eficiencia capturable','60'],['Coste atribuible de la iniciativa','8.000'],['Coste de capital o referencia financiera','12']];
                  document.querySelectorAll('.field label').forEach(function(label){ var input = label.querySelector('input'); if (!input) return; var text = label.textContent || ''; examples.forEach(function(item){ if (text.indexOf(item[0]) > -1 && input.getAttribute('placeholder') !== item[1]) input.setAttribute('placeholder', item[1]); }); });
                }
                function captureBusinessInputs(){
                  var data = {}; var map = [['Ingresos o margen potencial estimado','I'],['Porcentaje estimado que podria capturarse','R'],['Eficiencia economica potencial','E'],['Porcentaje estimado de eficiencia capturable','Q'],['Coste atribuible de la iniciativa','C'],['Coste de capital o referencia financiera','WACC']];
                  document.querySelectorAll('.field label').forEach(function(label){ var input = label.querySelector('input'); if (!input) return; var text = label.textContent || ''; map.forEach(function(item){ if (text.indexOf(item[0]) > -1 && input.value) data[item[1]] = input.value; }); });
                  if (Object.keys(data).length) { try { sessionStorage.setItem('docroi-kai-business-inputs', JSON.stringify(data)); } catch (e) {} }
                }
                ...
                function calculateDeclaredEconomics(){
                  var data = getBusinessInputs(); var I = readNumber(data.I); var R = rateFromPercentText(data.R); var E = readNumber(data.E); var Q = rateFromPercentText(data.Q); var C = readNumber(data.C); var WACC = rateFromPercentText(data.WACC);
                  var md = I !== null && R !== null && E !== null && Q !== null ? (I * R + E * Q) : null;
                  return { I:I, R:R, E:E, Q:Q, C:C, WACC:WACC, MD:md, marginRate: I && md !== null ? md / I : null };
                }
                function convertCustomerEquityToMultiples(){
                  document.querySelectorAll('.metrics .card').forEach(function(card){
                    var title = card.querySelector('strong'); var value = card.querySelector('span'); var note = card.querySelector('p.small'); if (!title || !value) return;
                    var label = title.textContent.trim(); if (label !== 'Customer Equity' && label !== 'Customer Equity empresa') return;
                    var raw = value.textContent.trim(); if (!raw || raw.indexOf('%') === -1) return; var percent = readNumber(raw); if (!Number.isFinite(percent)) return;
                    value.textContent = formatMultiple(percent / 100);
                    if (label === 'Customer Equity' && note) note.textContent = percent < 0 ? 'Veces que el ROI estimado queda por debajo del WACC. Un valor negativo indica que el escenario todavia no crea Customer Equity.' : 'Veces que el ROI estimado queda por encima del WACC. Ejemplo: 3,5x significa 3,5 veces sobre la referencia financiera.';
                    else if (note) note.textContent = percent < 0 ? 'Suma ejecutiva negativa de las unidades evaluadas: el escenario todavia no supera la referencia financiera.' : 'Suma ejecutiva de unidades evaluadas, expresada como veces sobre WACC en esta version del diagnostico.';
                  });
                }
                ...
                function preparePdfActions(){
                  var topActions = document.querySelector('.hero .actions.noPrint'); if (topActions) { var buttons = topActions.querySelectorAll('.btn'); buttons.forEach(function(button, index){ if (index === 0) { button.innerHTML = 'Descargar diagnóstico en PDF'; button.setAttribute('aria-label','Descargar diagnóstico en PDF'); } else button.style.display = 'none'; }); }
